...
import static bg.softuni.util.handler.EnumValidator.validateEnum;

@Service
public class UserServiceImpl implements UserService {
    private final String AMOUNT_ERROR = "Amount must be between 0.01 and 5 000 000 euro!";
    private final String USER_ERROR = "User with that username not found!";
    ...
    public UserServiceModel registerUser(RegisterUserModel userModel) {
        ...
        this.newNotification(saved, String.format("Hello %s %s, welcome to our site!", saved.getFirstName(), saved.getLastName()));
    ...
    public void withdraw(String username, MoneyTransactionModel model) {
        ...
        if (BigDecimal.ZERO.compareTo(user.getBalance().subtract(model.getAmount())) > 0) {
            throw new IllegalStateException(String.format("User doesn't have enough money to withdraw %s euro - balance: %s €"
    ...
    public UserServiceModel changeRoles(EditRolesModel model) {
        ...
        this.newNotification(saved
                , "You roles have been changed by an administrator - roles: "
    ...
    private void validateEntity(RegisterUserModel userModel) throws IllegalStateException {
        ...
        validateEnum(userModel.getGender(), Gender.class, "");//Check if enum value is valid
    }
